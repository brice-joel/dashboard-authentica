import React, { useState, useEffect } from "react";

const SelectCascade = ({
    name,
    options,
    placeholder = "Sélectionnez une option",
    onChange,
    className = "",
    defaultValue = "",
    required = false,
    error = false,
}) => {
    // État pour la valeur sélectionnée
    const [selectedValue, setSelectedValue] = useState(defaultValue);

    // Ce hook se déclenche chaque fois que la prop `defaultValue` change
    useEffect(() => {
        setSelectedValue(defaultValue);
    }, [defaultValue]);

    const handleChange = (e) => {
        const value = e.target.value;
        setSelectedValue(value);
        onChange(value); // Déclenche le gestionnaire parent
    };

    return (
        <>
            <select
                className={`h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 pr-11 text-sm shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800
                ${
                    selectedValue
                        ? "text-gray-800 dark:text-white/90"
                        : "text-gray-400 dark:text-gray-400"
                } ${className}`}
                name={name}
                value={selectedValue}
                onChange={handleChange}
                required={required}
            >
                {/* Option de placeholder */}
                <option
                    value=""
                    disabled
                    className="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                >
                    {placeholder}
                </option>

                {/* Cartographie sur les options de données en cascade */}
                {Object.keys(options).map((group) => (
                    <optgroup
                        key={group}
                        label={group}
                        className="text-gray-800 dark:bg-gray-800 dark:text-white"
                    >
                        {options[group].map((option) => (
                            <option
                                key={option.value}
                                value={option.value}
                                className="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                            >
                                {option.label}
                            </option>
                        ))}
                    </optgroup>
                ))}
            </select>

            {error && <p className="mt-1.5 text-xs text-error-500">{error}</p>}
        </>
    );
};

export default SelectCascade;
